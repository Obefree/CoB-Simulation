...
def fix_nan(obj):
    if isinstance(obj, dict):
        ...
        if obj.lower() == 'nan' or obj.lower() == 'inf':
            return None
        return obj
    elif obj is None:
        return None
...
def parse_main_cards():
    ...
    for _, row in df.iterrows():
        ...
        card = {
            ...
            'cost': int(str(row.get('Cost_Bless', '0')).replace(',','').split()[0]) if str(row.get('Cost_Bless', '0')).strip() else 0,
            'effect1': effect1,
            'effect1text': effect1text,
            'effect2': effect2,
            'effect2text': effect2text,
            'copies': int(row.get('Copies', 1)) if str(row.get('Copies', '')).strip() else 1,
            'raw': fix_nan(row.to_dict())
        }
    ...
    for _, row in df.iterrows():
        ...
        try:
            if cost_val is None or str(cost_val).strip() == '' or str(cost_val).lower() == 'nan':
                ...
                cost = int(str(cost_val).replace(',','').split()[0])
        except Exception:
            cost = 0
        card = {
            'name': str(row.get('Name', '')).strip(),
...
def simulate():
    try:
        ...
        num_games = int(data.get('num_games', 1000))
        ...
        for _ in range(num_games):
            try:
                collect_log = (num_games == 1)
                res = simulate_game(strategy1, strategy2, log=False, custom_hp=(hp1, hp2), collect_log=collect_log)
            except Exception as e:
        ...
        # Топ gear (всегда включаем Priestess, даже если absorbed_damage=0)
        if 'Priestess' not in gear_absorb:
            gear_absorb['Priestess'] = 0
        ...
        with open(f'simulation_result_{timestamp}.json', 'w', encoding='utf-8') as f:
